...
app.get('/api/users', authenticateToken, async (req, res) => {
  try {
    const result = await pool.query('SELECT id, name, email, theme, is_deleted, deleted_at, created_at FROM users ORDER BY created_at DESC');
...
app.get('/api/users/:id', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query('SELECT id, name, email, theme, is_deleted, deleted_at, created_at FROM users WHERE id = $1', [userId]);
...
app.post('/api/users/restore', authenticateToken, async (req, res) => {
  try {
    await pool.query('UPDATE users SET is_deleted = FALSE, deleted_at = NULL WHERE id = $1', [req.user.id]);
...
app.delete('/api/users', authenticateToken, async (req, res) => {
  try {
    await pool.query('UPDATE users SET is_deleted = TRUE, deleted_at = NOW() WHERE id = $1', [req.user.id]);
...
app.post('/api/quiz/start', authLimiterMiddleware, authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO quiz_sessions (user_id, difficulty, category, score, time_left, total_questions) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id',
      [req.user.id, difficulty, category || 'mixed', 0, settings.time, numQuestions]
    );
    ...
    for (let i = 0; i < questions.length; i++) {
      await pool.query(
        'INSERT INTO quiz_questions (session_id, question_number, question, correct_answer) VALUES ($1, $2, $3, $4)',
...
app.post('/api/quiz/answer', authLimiterMiddleware, authenticateToken, async (req, res) => {
  try {
    ...
    const questions = await pool.query(
      'SELECT id, correct_answer FROM quiz_questions WHERE session_id = $1 AND question_number = $2',
    ...
    if (isCorrect) {
      await pool.query(
        'UPDATE quiz_sessions SET score = score + 10 WHERE id = $1',
        [sessionId]
      );
    }
...
app.post('/api/quiz/submit', authLimiterMiddleware, authenticateToken, async (req, res) => {
  try {
    ...
    const sessions = await pool.query(
      'SELECT score, time_left FROM quiz_sessions WHERE id = $1 AND user_id = $2',
      [sessionId, req.user.id]
    );
    ...
    const finalScore = sessions.rows[0].score;

    await pool.query(
      'UPDATE quiz_sessions SET score = $1, time_left = $2 WHERE id = $3',
      [finalScore, timeLeft, sessionId]
    );
    ...
    await pool.query(`
      ...
      ON CONFLICT (user_id) DO UPDATE SET
        ...
        average_score = (user_stats.total_score + $2) / (user_stats.total_games + 1)
    `, [req.user.id, finalScore, finalScore]);
...
app.get('/api/quiz/history', authenticateToken, async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT * FROM quiz_sessions WHERE user_id = $1 ORDER BY created_at DESC',
      [req.user.id]
    );
    res.json(result.rows);
  } catch (error) {
...
app.delete('/api/quiz/session/:id', authenticateToken, async (req, res) => {
  try {
    ...
    await pool.query('DELETE FROM quiz_sessions WHERE id = $1 AND user_id = $2', [sessionId, req.user.id]);
...
app.get('/api/quiz/session/:id/questions', authenticateToken, async (req, res) => {
  try {
    ...
    const sessions = await pool.query(
      'SELECT qs.*, u.name as user_name FROM quiz_sessions qs JOIN users u ON qs.user_id = u.id WHERE qs.id = $1',
      [sessionId]
    );

    const questions = await pool.query(
      'SELECT * FROM quiz_questions WHERE session_id = $1 ORDER BY question_number',
      [sessionId]
    );

    res.json({ session: sessions.rows[0], questions: questions.rows });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
app.delete('/api/quiz/session/:id/remove', authenticateToken, async (req, res) => {
  try {
    ...
    const deletedSession = sessions.rows[0];

    await pool.query('DELETE FROM quiz_sessions WHERE id = $1', [sessionId]);

    // Recalculate stats
    const remainingSessions = await pool.query(
      'SELECT SUM(score) as total_score, COUNT(*) as games_count FROM quiz_sessions WHERE user_id = $1',
      [userId]
    );

    const totalScore = remainingSessions.rows[0].total_score || 0;
    const totalGames = parseInt(remainingSessions.rows[0].games_count) || 0;
...
app.get('/api/achievements', authenticateToken, async (req, res) => {
  try {
    ...
    const achievements = allAchievements.rows.map(ach => ({
      ...ach,
    ...
    for (let i = 0; i < achievements.length; i++) {
      if (achievements[i].earned) {
        ...
        if (earned.rows.length > 0) {
          achievements[i].earnedAt = earned.rows[0].earned_at;
        }
      }
    }
...
app.post('/api/achievements/check', authenticateToken, async (req, res) => {
  try {
    ...
    let stats = await pool.query('SELECT * FROM user_stats WHERE user_id = $1', [userId]);

    if (stats.rows.length === 0) {
      await initUserStats(userId);
      stats = await pool.query('SELECT * FROM user_stats WHERE user_id = $1', [userId]);
    }

    const userStats = stats.rows[0];

    const achievements = await pool.query(
      'SELECT * FROM achievements WHERE requirement_type = $1 AND requirement_value <= $2',
...
app.get('/api/profile', authenticateToken, async (req, res) => {
  try {
    ...
    const user = await pool.query('SELECT id, name, email, theme FROM users WHERE id = $1', [userId]);
